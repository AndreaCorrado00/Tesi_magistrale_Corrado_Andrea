def tune_relative_his_th_on_f1(signals, y_true,interval,t_atr, t_ven, plot=False):
    ...
    for idx, Q_perc in enumerate(interval):
        ...
        his_bundle_th=tune_his_th_on_ampl(signals, t_atr, t_ven, Q_perc,False)
        
        for i in range(dims[0]):
            _, _, _, pred = heuristic_classifier_B_2(signals.iloc[i], 2035, his_bundle_th)
    ...
    his_bundle_th=tune_his_th_on_ampl(signals, t_atr, t_ven, max_f1_C_Q_perc[0],False)
    
    return his_bundle_th,max_f1_C_Q_perc[0]